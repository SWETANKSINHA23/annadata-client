  ...
  const checkNearbyCustomers = async (location: { lat: number; lng: number }) => {
    try {
      ...
      if (response.data && Array.isArray(response.data.consumers)) {
        setNearbyConsumers(response.data.consumers);
        setNearbyConsumersCount(response.data.consumers.length);
        
        console.log(`Found ${response.data.consumers.length} nearby consumers`);
        
        // Show notification for newly discovered consumers
        if (response.data.consumers.length > 0) {
          toast({
            title: "Consumers Nearby!",
            description: `${response.data.consumers.length} consumers are in your area.`,
          });
        }
      }
    } catch (error) {
            ...
            <CardContent>
              <div className="space-y-6">
                ...
                <div className="text-center mb-4">
                  <div className="text-sm text-gray-600">{ratingStats.totalRatings} total ratings</div>
                </div>
                <div className="space-y-2">
