...
interface User {
  ...
  name: string;
  email: string;
  role: "farmer" | "vendor" | "consumer" | "admin";
  phone: string;
}
...
const Login = () => {
  ...
  const [userType, setUserType] = useState<"farmer" | "vendor" | "consumer" | "admin">("farmer");
  ...
  const handleUserTypeChange = (value: "farmer" | "vendor" | "consumer" | "admin") => {
    setUserType(value);
  };
  ...
  const handleLoginAfterVerification = async () => {
    ...
    try {
      ...
      const response = await api.post(`/auth/login/${userType}`, {
      ...
      console.log("User login successful with role:", user.role);
      ...
      if (user.role === 'admin') {
        console.log("After verification - Redirecting admin user to: /admin/dashboard");
        navigate('/admin/dashboard');
      } else {
        console.log(`After verification - Redirecting ${user.role} user to: /dashboard/${user.role}`);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (formData.rememberMe) {
        ...
        localStorage.setItem('rememberedCredential', formData[loginMethod]);
      ...
      if (user.role === 'admin') {
        ...
        console.log(`Redirecting ${user.role} user to: /dashboard/${user.role}`);
        navigate(`/dashboard/${user.role}`);
      }
    } catch (error: any) {
      console.error("Login error:", error);
  ...
  useEffect(() => {
    ...
    const rememberedUserType = localStorage.getItem('rememberedUserType') as "farmer" | "vendor" | "consumer" | "admin" | null;
                  ...
                  <form onSubmit={handleSubmit} className="space-y-6 pt-6">
                    ...
                    <div className="space-y-3">
                      ...
                      >
                        ...
                        <div className={`flex flex-col items-center justify-center border rounded-lg p-4 cursor-pointer transition-colors ${userType === "admin" ? "border-[#FF0000] bg-[#FF0000]/5" : "border-gray-200"}`}>
                          <RadioGroupItem value="admin" id="login-admin" className="sr-only" />
                          <Label htmlFor="login-admin" className="cursor-pointer flex flex-col items-center gap-2">
      ...
      <Dialog open={showOTPDialog} onOpenChange={setShowOTPDialog}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Your email is not verified. Please enter the verification code sent to {verificationEmail}.
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-4 py-4">
            ...
            <div className="flex justify-center">
              ...
              >
                {resendingOTP ? "Sending..." : "Didn't receive the code? Resend OTP"}
              </Button>
            </div>
          </div>
        </DialogContent>
